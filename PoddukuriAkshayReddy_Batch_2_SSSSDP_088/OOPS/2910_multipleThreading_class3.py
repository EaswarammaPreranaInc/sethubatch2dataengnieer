...
f1 finally
Outside finally
raise NameError() # which is not handled 
...
''''
except without  try  is  syntax  error
'''
...
# 2 two else blocks  are  not  allowed
...
# except block  should  come  before  else  block
...
'''  (Home  work)
1) What  is  the  output  if  input  is  24 ?  ---> Hyd and End

2) What  is  the  output  if  input  is  25 ?  ---> Sec and End
'''
try:
...
2) What  is  the  output  when  input  is  25 ?  ---> Sec and End 
'''
try:
	x = eval(input('Enter  any  number  :  '))
	assert   x >= 25
...
Inner  try  finally
Outer  try  finally
Type Error which is not caught or handled
...
try:
	...
	print('Caught  MyError  outside  :  ' ,  msg)
print('End')
[29-10-2025 11:46] SRINIVAS Sir SSSSDP: # Find  outputs (Home  work)
try:
	print(1)
	print(2)
	print(3)
...
try:
	...
	int('Two')
        ...
        print(6)
print(7)
[29-10-2025 13:36] SRINIVAS Sir SSSSDP: # Find  outputs (Home  work)
...
'''
infinite  printing  of of  s: 10 and s: 20 with  thread  names  Hyd  and  Sec
...
# main   thread is  dead
'''
due to join it will separate the new thread and main thread outputs
'''
...
for  i  in  range(10):
	print('main  thread')
'''
4 new threads and 10 main threads will be printed because new.join(10)  will  wait  for  10  seconds  for  new  thread  to  complete
6 new threads  will  be  printed  after  that  main  thread  will  print  its  outputs
'''
...
Three is started
4
Two One Three are ended in any order
1
'''
