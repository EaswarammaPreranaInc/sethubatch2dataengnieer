...
# Identify  error  (Home  work)
        ...
        print('Sec')
        print('Cyb')
# Try and except  will go in pair
...
# there should be 2 except blocks with specific exception names
...
print(int('10.8')) # yes
print(float('Ten')) # yes
print(complex('True')) # yes
print(bool('Ten')) # no
print(bool('')) # no
print(float('10.8')) # no
print(float('25')) # no
print(int(10.8)) # no
print(math . sqrt(-25)) # yes
...
b = { [10 , 20] : [30 , 40] } # yes 
print(int(3 + 4j)) # yes
print(int([10 , 20 , 30])) # yes
print(float(None)) # yes
...
2 --> Invalid  index
3 --> No  result
4 --> Invalid   argument (or)  operand
5 --> Object  does  not  exist
6 --> Div by 0 is not allowed
7 --> Invalid   argument (or)  operand
8 --> Invalid dict key
9 --> Bye
'''
...
for i in  range(10):
    print('Main  Thread')
child . start() # error only once we can start a thread
'''
Child  Thread
Child  Thread
Main  Thread
...
for  i  in  range(1 , 11):
        print('Main  Thread  :  ' , i)
'''output are not predictable because child thread may start before (or) after main thread'''
...
class   Thread:
        def   run(self):
                for  i  in  range(10):
                        print('Child  Thread')
# End of the class
t = Thread() # the Thread refers to the Thread class defined by the user not the threading module
t . start()
for  i  in  range(10):
        print('main  thread')
...
'''
f1 function : i
Main Thread : i in interleaved manner
'''
