...
print(sum(d))   # 63
e = [25 , 10.8 , 'Hyd' , True]
print(sum(e))   # Error because 'Hyd' is not a number
...
a = [[10 , 20 , 15 , 18]]
print(sum(a)) #Error bacause sum() can't be used for list of lists
print(sum(a[0]))  # Output: 63
print(sum([item for i in a for item in i]))  # Output: 63
...
print(min(b))      # Amar
c = [25 , 10.8 ,  3 + 4j , True]
print(max(c))      # Error: can't compare complex numbers
d = [25 , '35']
print(max(d))      # Error: can't compare int and str
...
print(list(None))     #Error: 'NoneType' object is not iterable
...
try:
	...
	list . remove(25)   #Error because 25 is not in the list
except:
...
input_element = int(input('Enter  element  to  be  deleted : '))
for i in list:
	if i == input_element:
		list . remove(i)
print(f'List  without   {input_element}', list)
...
a = [25 , 10.8 ,  'Hyd' ,  True]
a . sort()   #Error because mixed sequence 
...
'''
Write a program to determine if all the list elements are identical or not

Hint: Use len() and count()
'''
lst = eval(input("Enter any list: "))
if len(lst) > 0 and lst.count(lst[0]) == len(lst):
