...
if len(argv)==1:
	...
	if st==True and num==False:
		print('Largest input : ',max(strlist))
	elif st==False and num==True :
		print('Largest input : ',max(numlist))
	else:
		print('Both integer and string are not acceptable at a time')
...
if len(argv)==1:
	...
	for i in argv[1:]:
		try:
			...
			st=True
	if num == True and st == False:
		print('Average : ', sum(numlist)/(len(argv)-1))	
	else:
		print('Input can\'t be strig')
...
if len(argv)==1:
	...
	for i in argv[1:]:
		try:
			...
			st=True
	if num == True and st == False:
		print('Ascending order of inputs : ',sorted(numlist))
		print('Descending order of inputs : ',sorted(numlist, reverse=True))
	elif num == False and st == True :
		print('Ascending order of inputs : ',sorted(strlist))
		print('Descending order of inputs : ',sorted(strlist, reverse=True))
	else:
		print('Input can\'t be strig and number')
...
e_str=input("Enter  any  string  : ")
if len(e_str)<4:
  print('')
else:
  print(e_str[:2]+e_str[-2:])
...
for i in range(len(ip)):
  print(f'Character at index {i} : {ip[i]}')
...
for i in range(len(ip)):
  if i%2 == 0:
    ...
    o_str+=ip[i]
print('Characters at even indexes : ',e_str)
print('Characters at even indexes : ',o_str)
...
e_str=''
ip=input('Enter  any  string  with  alternate  character  and  digit : ')
if len(ip)>=2:
  for i in range(0,len(ip),2):
    if ip[i+1].isdigit():
      e_str+=ip[i]*int(ip[i+1])
    else:
...
l=min([len(ip1),len(ip2)])
e_str=''
for i in range(l):
  e_str+=ip1[i]+ip2[i]
if len(ip1)<len(ip2):  
  print('Result : ',e_str+ip2[l:])
else:
  print('Result : ',e_str+ip1[l:])
...
ip, e_str=input("Enter  any  string  : "), ''
for i in ip:
	if i not in e_str:
		e_str+=i
print('String  without  duplicates  : ',e_str)
...
print(len(' '))           #1
print(len('A2#'))         #3
print(len(3456))         #Returns error as len is not applicable for int class objects
print('Sec'. len())       #Returns error as it must be len('Sec')
...
for i in range(0,len(ip)-1,2):
	if ip[i+1].isdigit():
		e_str+=ip[i]+chr(ord(ip[i])+int(ip[i+1]))
	else:
