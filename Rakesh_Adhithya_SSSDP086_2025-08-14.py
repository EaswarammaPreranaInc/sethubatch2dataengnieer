...
ch0 = string[0]
print( ch0 + string[1:].replace(ch0, '*') )
...
# Find  outputs  (Home  work)
a = 'Hyd	is	green	city' # There  is  tab  between  each  word
print(a . split('\t'))        # ['Hyd', 'is', 'green', 'city']
print(a . split())            # ['Hyd', 'is', 'green', 'city']
print(a . split(' '))         # ['Hyd\tis\tgreen\tcity']
...
e = [15 , 36 , 48]
#print(':' . join(e))                    #TypeError: sequence must contain only strings
f = ['Sankar' , 'Dayal' , 'Sarma']
print('' . join(f))                      #SankarDayalSarma
g = range(5)
#print('-' . join(g))                     #TypeError: range object has only integers
...
print(a . endswith('green' , 3 , 12))    #True  (end is including)
...
print('' . isdigit())       #False
print(' ' . isdigit())      #False
#print(9247 . isdigit())    #AttributeError, isdigit is a method of string not int
...
3) i        b[-i]           c
   ...
   --------------------------------------------------------
'''
words = input('Enter any sentence:  ').split()
reverse = ''
...
alnum = input('Enter any alphanumeric string:  ')
sorted_alnum = sorted(alnum)
i = 0
while 48 <= ord(sorted_alnum[i]) <= 57:
    i += 1
sorted_alnum = sorted_alnum[i:] + sorted_alnum[:i]
print(''.join(sorted_alnum))
