#1. Write  a  program  to  determine  frequency  of  each  vowel  in  the  string  in  alphabetical  order (Ignoring  the  case)
a = (input("Enter a string  :  ")).upper() # 'Rama Rao'
b = sorted(a)
c ={}
for i in b:
    if i.isalpha() and i in 'AEIOU':
        c[i] = c . get(i , 0) + 1
print(c) # {'A': 3, 'O': 1}






#2. Find outputs (Home  work)
a = [ ('R' , 'Red') , ('G' , 'Green') , ('B' , 'Blue')]
b = {'Y' : 'Yellow', 'G' : 'Gray'}
b . update(a) 
print(b) # {'Y' : 'Yellow', 'G' : 'Green', 'R' : 'Red', 'B' : 'Blue'}
#a . update(b) # Error due to list does not have update method


#3.  Find  outputs  (Home  work)
a = [ (10 , 20 , 30) , (40 , 50 , 60) , (70 , 80 , 90)]
b = {}
#b.update(a) # Error
print(b) # {}
c = [(10,) , (20,) , (30,)]
#b . update(c) # Error
print(b) # {}



#4.  Find  outputs (Home  work)
d = {x : x ** 3   for    x   in  range(5)}
print(d) # {0 : 0, 1:1, 2:8, 3:27, 4:64}
print(type(d)) # <class 'dict'>





#5. Find outputs   (Home  work)
d = { x :  2 * x    for    x   in   range(5) }
print(d) # { 0 : 0, 1 : 2, 2 : 4, 3 : 6, 4 : 8}





#6. Find outputs  (Home  work)
a = {10 : 'Rama' , 15 : 'Sita' , 18 : 'Rajesh' , 17 : 'Kiran' , 12 : 'Rama Rao'}
b = { k :  v  for   k , v  in   a .items()   if    k % 2 != 0}
print(b) # {15 : 'Sita',17 : 'Kiran'}
c = {k : a[k]   for   k   in    a    if   a[k] . startswith('R')}
print(c) # {10 : 'rama', 18 : 'Rajesh' , 12: 'Rama Rao'}








#7. Find outputs  (Home  work)
def   f1():
	print('Hyd')
	print('Sec')
	print('Cyb')
# End  of  the  function
print('Begin')
x = f1()
print(x) # None
print(type(x))
print('End')
# Output:
# Begin
# Hyd
# Sec
# Cyb
# None
# <class 'NoneType'>
# End





#8. Find  outputs  (Home  work)
def  f1():
	return  10 , 20 , 30
# End  of  the  function
x = f1()
print(x) # (10 ,20, 30)<nextline>None
print(type(x)) # <class 'NoneType'>
a , b , c =  f1()
print(a) # 10
print(b) # 20
print(c) # 30
print('for  loop') # for loop
for  k   in   f1():
	print(k) # 10<nextline>20<nextline>30






#9. Find  outputs
def    f1():
        return  10
        return  20
        return  30
# End  of  the  function
print('Begin') # Begin
x = f1()
print(x) # 10
print('End') # End
#return   100 # Error due to it is at outside of function







#10.  Find  outputs
f1()
def   f1():
        print('Hello')
print(f1()) # Hello<nextline>None
print(f1) # type and address of function f1





#11. Find  outputs  (Home  work)
print('Hello') # Hello
def  f1():
        print('f1  function')
#End  of   function
print('Hi') # Hi
print(f1()) # fl function <nextline> None
print(f1) # Type and address of function f1
print('Bye') # Bye





#12.  Find  outputs
def    f1():
        print('Hyd')
        print('Sec')
        print('Cyb')
#End  of  the  function
print('Begin') # Begin
print(type(f1)) # <class 'function'>
print(id(f1)) # Address of finction f1
print('End') # End







#13.  Find  outputs (Home  work)
def   add(a , b):
        return  a + b
#End  of  the  function
print(add(10 , 20)) # 30
print(add('Hyder' , 'abad')) # Hyderabad
print(add(10.8 , 20.6)) # 31.4
print(add(True , False)) # 1
print(add(3 + 4j , 5 + 6j)) # 8 + 10j
print(add(25 , 10.8)) # 35.8
print(add([25 , 10.8 , 'Hyd'] , [True , None , 3+4j , 'Sec'])) # [25 , 10.8 , 'Hyd' , True , None , 3+4j , 'Sec']
#print(add(10 , '20')) # Error







#14. Find  outputs (Home  work)
def  f1():
	print('No-argument  function')
def  f1(x):
	print('Single  argument  function  : ' , x)
def  f1(x , y):
	print('Two  argument  function : ' , x , y)
def  f1(x , y , z):
	print('Three  argument  function : ' , x , y , z)
f1(10 , 20 , 30) # Three  argument  function : 10 20 30
#f1(40 , 50) # Error due to as same function for all functions ,the last function we executed and remaining discarded
#f1(60) # # Error due to as same function for all functions ,the last function we executed and remaining discarded# Error due to as same function for all functions ,the last function we executed and remaining discarded
#f1() # # Error due to as same function for all functions ,the last function we executed and remaining discarded







# 15. Write   a  function  to  test  a  number  is  prime  (or)  not.
def prime(n):
    if not isinstance(n, int):
        return "Invalid input"
    if n <= 1:
        return "Composite number"
    for i in range(2, (n // 2) + 1):
        if n % i == 0:
            return "Composite number"
    return ("Prime number")
k = int(input("Enter any number : "))
n = prime(k)
print(n)



#16. Find  outputs  (Home  work)
def   disp(empno , ename , sal):
        print(F'Emp  Number  :  {empno} \t  Emp Name  :  {ename} \t  Salary  :  {sal}')
# End  of  the  function
disp(25 , 'Rama  Rao' , 10000.0) # Emp  Number  :  25 	  Emp Name  :  Rama  Rao 	  Salary  :  10000.0 
disp('Sita' , 20000.0 , 35) #Emp  Number  :  Sita 	  Emp Name  :  20000.0 	  Salary  :  35