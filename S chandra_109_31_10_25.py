...
(Exact order can vary depending on scheduling)
...
This is a race condition — final balance may become 1100.0 or 1200.0.





To Fix Race Condition (optional)

Use a Lock for thread synchronization:
...
class Account:
    ...
    def credit(self, amt):
        s = current_thread().name
        print(f'{s} is depositing Rs. {amt} into account {self.acno}')
        with self.lock:        # Lock ensures safe update
            x = self.bal
            time.sleep(1)
            self.bal = x + amt
...
zip() with no arguments gives empty iterator → no output.
