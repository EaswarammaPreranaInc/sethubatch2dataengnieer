...
def   bold(fun): # inner2
	def  inner1():  
		return  '<b>'+  fun()  +'</b>' # '<b>'+  inner2()  +'</b>' # '<b>'+'<i>'+'<u>'+ 'Hello World' +'</u>'+'</i>'  +'</b>'
	return  inner1
def   italic(fun): # fun = inner3
	def   inner2():
		return  '<i>'  +  fun() +  '</i>' # '<i>'+  inner3 +'</i>' # '<i>'+'<u>'+  'Hello World'  +'</u>' +'</i>'
	return  inner2 # inner2
def   underline(fun): # fun = f1
	def   inner3():
		return  '<u>'  +  fun()  +  '</u>' # '<u>'  +  f1  +  '</u>' # '<u>'  +  'Hello World'  +  '</u>'
...
def   f1():
       return  'Hello  World'
# End  of  the  function
print(f1()) # print(inner1()) # '<b>'+'<i>'+'<u>'+  'Hello World'  +'</u>'+'</i>'+'</b>'
