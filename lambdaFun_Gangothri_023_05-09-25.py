...
a  =  lambda  :  'Hyd'
print(a()) # Hyd
print(a) # Type address of function
...
9
243
<function f1.<locals>.<lambda> at 0x0000017CCF41DEE0> '''
...
while n > 1:
    ...
    while user < 1 or user > 4 or user > n:
        user = int(input("Invalid choice! Reenter (1-4): "))
    n -= user
    print(f"User picked {user}, remaining = {n}")
    if n == 1:   
        print("User is forced to pick the last stick. Computer wins ")
        break
    computer = 5 - user
    if computer > n:  
        computer = n - 1
    n -= computer
    print(f"Computer picks {computer}, remaining = {n}")
    if n == 1:  
        print("Computer is forced to pick the last stick. User wins ")
        break
