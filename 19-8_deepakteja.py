...
False
'''
# Program: Check if first list is a sublist of second list
...
try:
    pos = -1
    for x in first:
        pos = second.index(x, pos + 1)   # find next occurrence after pos
    print(True)   # loop completed without error
except ValueError:
    print(False)  # element not found in order
...
for x in set(a):                  # unique elements
    freq = a.count(x)             # count how many times element repeats
    if freq > ctr:                # check for maximum
        mode = x
        ctr = freq
...
print([1][3])                      #(How  to  print  80)
print([2][1])                      #(How  to  print  100)
...
for inner in a:
  print(inner)                                                             #(How  to  print  inner  list  in  another  way)
