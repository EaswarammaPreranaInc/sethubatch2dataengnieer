...
#Program:
#Program:
def fib(i):  # 'i' is term number (0-based)
    if i == 0:
        return 0
    if i == 1:
        return 1
...
#Output:
Error, because there is a local variable in f1() function, so 3 is the output everytime, loop is not stopped.
...
f1()						# Error, because functions are repetadely executes
