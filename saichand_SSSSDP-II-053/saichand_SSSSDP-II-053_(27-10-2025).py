...
try:					# Error, We cannot use only the try block
	print('Hyd')
	print('Sec')
	print('Cyb')
...
# Identify  error  (Home  work)
except:						# Error, We cannot use only the except block
        print('Hyd')
        print('Sec')
        print('Cyb')
...
except NameError:			# we cannot use NameError after the default except, default except is at last
	print('Name  Error')
...
except:						# Error, there is only one default except we can use
...
#Output: (output can be in any order)
10 times child thread
10 times main thread
...
child = Thread(target=c1.m1)					# child = Thread(How  to  specify  the  target  as  class  method)
child . start()
for  i  in  range(1 , 11):
        print('Main  Thread  :  ' , i)
