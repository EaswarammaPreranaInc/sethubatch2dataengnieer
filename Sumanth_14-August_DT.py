...
print(modify_string("interest"))     # interesting
print(modify_string("interesting"))  # interestingly
print(modify_string("Hi"))           # Hi
...
'''
# Program
# Program to determine type of character
...
elif ch.isspace():  # Space, Tab, Enter
    print("White space")

else:  # Not alphanumeric and not space → special char
...
b = ""  # Empty string to build the reverse
for i in range(1, len(a)+ 1):   # i goes from 1 to length
    b = b + a[-i]  # take from the end using negative index
...
b = a.split()  # Split into words list
c = ""         # Empty string to store reversed sentence

length = len(b)
for i in range(1, length + 1):  # i goes from 1 to length
    c = c + b[-i] + " "
...
c = ""          # To store final result

for word in b:  # For each word in list
    c = c + word[::-1] + " "  # Reverse using slice and add
...
sorted_str = ''.join(sorted(a))  # sorted() returns list of characters in order
...
# Step 1: Sort the string normally
sorted_str = sorted(a)  # Gives characters in order
...
# Step 3: Concatenate alphabets first, then digits
result = alpha + digit

print("Sorted (alphabets first, digits next):", result)
