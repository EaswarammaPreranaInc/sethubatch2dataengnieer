...
import mysql.connector as mc
try:
    con = mc.connect(host='localhost',user='root',password='',database='empdb')
    cur = con.cursor()
    while True:
        ...
        try:
            ...
            print("Duplicate empno and row cannot be inserted")
...
Enter employee name:Ammulu
Enter salary:15000
Duplicate empno and row cannot be inserted
...
try:
    ...
    con.commit()
    print(F'{cur.rowcount} Row updated')
    cur.close()
    con.close()
except mc.errors.ProgrammingError as msg:
    print("Error:", msg)
