...
try:
    first = eval(input("Enter the first list : ")) # [10,20,30]
    second = eval(input("Enter the second list : ")) #[15 , 18 , 10 ,  12 ,  19 , 20 , 14 , 12 , 30 , 25 ,  16]
    k = 0
    for i in first:
        if i in second[k:]:
            k = second.index(i,k+1)
    print("True")   
except:
    print('False')
...
#4  Nested  List  demo  program  (Home  work)
...
#6  How  to  print  nested  list  in  differnent  ways
a = [[10 , 20] , [30 , 40 ,  50] , [60 , 70 , 80 , 90]]
...
#for  x , y  in  a:
#	print(x , y ,	sep = '...') # Error due to to many values to unpack
