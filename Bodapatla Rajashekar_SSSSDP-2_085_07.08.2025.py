#1. program to determine largest commmand line argument
...
#2. Even or odd number of command line inputs
from sys import argv
try:
    s = []
...
#3. average of command line inputs
from sys import argv
try:
    a=[]
    for i in range(1,len(argv)):
        try:
            ...
            if argv[i].isalpha():
                a.append((argv[i]))
            if not argv[i].isalpha() and not argv[i].isdecimal():
                print("Inputs cannot be number and string")
    print(max(a))
except ValueError:
    print("Please send atlease one input")



#4. sort commamnd line inputs in ascending and descending order
from sys import argv
try:
    s = []
    for i in range(1,len(argv)):
...
#9.write a program to determine forward and backward diretion 
a = input("Enter a string :")
...
#10. print character at even and odd indexes 
a = input("Enter a string :")
even = ''
odd = ''
for i in range(len(a)):
...
#13. remove duplicates  without using set
