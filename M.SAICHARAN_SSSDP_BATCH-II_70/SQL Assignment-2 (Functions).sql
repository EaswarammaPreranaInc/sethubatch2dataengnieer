...
SELECT hire_date, QUARTER(hire_date) AS hire_quarter
FROM employees;

-- 4 Display hire dates with their corresponding week numbers.
SELECT hire_date,WEEK(hire_date) AS hire_week
FROM employees;
...
SELECT last_name, LEFT(last_name, 3) AS first3characters
FROM employees;

-- 3 Show names in upper and lower case.
SELECT first_name, LCASE(first_name) AS lower_case_name, uCASE(first_name) AS upper_case_name
FROM employees;
...
-- 10  employees whose last name starts with 'S'.
...
SELECT LN(1000) AS natural_log,
       LOG10(1000) AS base10_log;
...
SELECT salary,salary+floor(500+(rand()*1501)) AS salary_bonus 
FROM employees;
