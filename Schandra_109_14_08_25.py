...
while index != -1:
    print(index)
    index = a.rfind('is', 0, index)  # search to the left of current index
...
try:
    while True:
        index = a.rindex('is', 0, index)  # search right-to-left up to current position
        print(index)
except ValueError:
    print('End')
...
Hyd is green city. Hyd is hitec city. Hyd is his city
    ^^                 ^^                 ^^    ^^
    4,5               23,24              39,40 46,47

| Occurrence | Start Index | End Index | Context Around Match |
| ---------- | ----------- | --------- | -------------------- |
| 1          | 4           | 5         | `Hyd **is** green`   |
| 2          | 23          | 24        | `Hyd **is** hitec`   |
| 3          | 39          | 40        | `Hyd **is** his`     |
| 4          | 46          | 47        | `his **is** city`    |
...
if ch.isalnum():  # Alpha-numeric (letters or digits)
    print("Alpha Numeric Character")

    if ch.isalpha():  # Alphabet only
...
for i in range(1, len(s) + 1):  # Start from 1 to len(s)
    rev += s[-i]  # Get character from the end using negative index
...
for i in range(1, len(words) + 1):  # Start from last word
    rev += words[-i] + " "          # Append each word in reverse order with space

print("Reverse words :", rev.strip())  # Remove extra space at the end
...
for w in words:         # For each word
    rev += w[::-1] + " "  # Reverse using slice and add space

print("Reverse each word :", rev.strip())
...
#############
s = input("Enter any string : ")
sorted_str = "".join(sorted(s))  # sorted() returns a list, join() makes it a string
