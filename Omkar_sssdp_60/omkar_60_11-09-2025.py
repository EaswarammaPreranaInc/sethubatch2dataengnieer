...
# Find outputs (Home work)
def decor(fun):
    print(fun . name)  # Should be _name_ → Output: f1
    def inner():
...
# Find outputs(Home work)
def decor(fun):
    print(fun . name)  # Should be _name_ → Output: wish
    def inner(name):
...
def decor(fun):
    def inner():
        print(F'Decorating {fun . name} function')  # Should be _name_ → Output: Decorating f1 function
...
def decor(fun):
    print(fun . name)  # Should be _name_ → Output: f1, f2, f3, f4 (during decoration)
